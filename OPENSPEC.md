...
#### 7.3.3 專案區塊（Project Columns）標準內距

**定義**: 專案區塊（`.todo-column`）的標準內距（padding）規範

**桌面版** (≥ 768px):
```css
.todo-column {
  padding: 24px;  /* 桌面版標準內距 */
}
```
...
@media (max-width: 768px) {
  .todo-column {
    padding: 12px;  /* 手機版標準內距 */
  }
}
```

**規範說明**:
- 桌面版使用 `24px` 內距，提供充足的視覺空間
- 手機版使用 `12px` 內距，優化小螢幕空間利用
- 所有專案區塊（專案A、專案B、專案C）統一使用此標準
- 內距使用 `padding` 屬性，不包含 `margin`
